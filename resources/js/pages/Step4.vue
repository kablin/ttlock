...
const handleConnect = async () => {
  ...
  const isSuccess = email.value.includes('@') && password.value.length >= 6
  
  if (isSuccess) {
    notifications.value.push({ 
      id: Date.now(), 
      type: 'success', 
      message: 'Успешно. Найдено 25 замков' 
    })
  } else {
    notifications.value.push({ 
      id: Date.now(), 
      type: 'error', 
      message: 'Ошибка. Не правильно указан логин или пароль' 
    })
  }
...
<template>
  <div class="min-h-screen bg-white flex flex-col">
      ...
      <div class="space-y-8">
        ...
        <div class="flex flex-col items-center py-6">
          ...
          <div class="mb-6 text-center">
            <div class="w-20 h-20 rounded-full bg-blue-50 border-2 border-blue-200 flex items-center justify-center mb-2 mx-auto">
          ...
          <div class="w-full max-w-md space-y-4">
            <div class="space-y-2">
              ...
              <Input
                v-model="email"
                type="email"
                placeholder="Введите email от TTLock"
                class="bg-slate-50"
    ...
    <SetupNavigation
      ...
      :is-next-disabled="(!email.trim() || !password.trim()) && !hasSuccess"
