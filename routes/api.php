...
Route::middleware(['throttle:20,1'])->group(function () {
    ...
    Route::middleware([CodesCounter::class])->group(function () {
        ...
        Route::post('/v1/delete_code_from_lock', function (Request $request) {
            ...
            if (!isset(json_decode($request->getContent())->lock_id))     return response()->json(['status' => false, 'msg' => "Не указан lock_id"], 200);
            if (!isset(json_decode($request->getContent())->code_id))     return response()->json(['status' => false, 'msg' => "Не указан code_id"], 200);
    ...
    Route::post('/v1/add_code_packet', function (Request $request) {
        ...
        if (filter_var(json_decode($request->getContent())->codes_count, FILTER_VALIDATE_INT) === false) return response()->json(['status' => false, 'msg' => "codes_count не число"], 200);

        if (!isset(json_decode($request->getContent())->expired_at))     return response()->json(['status' => false, 'msg' => "Не указан expired_at"], 200);

        if (!DateTime::createFromFormat('Y-m-d H:i:s', json_decode($request->getContent())->expired_at) !== false) {
            return response()->json(['status' => false, 'msg' => "expired_at неверный формат даты"], 200);
        }
